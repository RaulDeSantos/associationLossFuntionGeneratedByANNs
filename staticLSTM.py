...
if len(sys.argv) > 1:
    ...
    wandb.init(name="perfectTracklets staticCamera", project="tfg", group="definitive static LSTM")

wandb.config.inputSize = 105
wandb.config.hiddenSize = 104
wandb.config.epochs = 100
wandb.config.learningRate = 0.0025
...
if len(sys.argv) > 1:
    wandb.config.modelSavingName = "modelPerfectTrackletsStaticCamera_" + sys.argv[1] + ".pt"
else:
    wandb.config.modelSavingName = "modelPerfectTrackletsStaticCamera.pt"
...
for trainingIdentity in [f.replace(".txt", "") for f in listdir(generalPath + r"\DATA\LSTMTRACKLET\staticCamera\train")
                         if isfile(join(generalPath + r"\DATA\LSTMTRACKLET\staticCamera\train", f))]:
...
for testingIdentity in [f.replace(".txt", "") for f in listdir(generalPath + r"\DATA\LSTMTRACKLET\staticCamera\test") if
                        isfile(join(generalPath + r"\DATA\LSTMTRACKLET\staticCamera\test", f))]:
...
for validationIdentity in [f.replace(".txt", "") for f in listdir(generalPath + r"\DATA\LSTMTRACKLET\staticCamera\val")
                           if isfile(join(generalPath + r"\DATA\LSTMTRACKLET\staticCamera\val", f))]:
...
def get_info(index: int, loadingState: int, normalize: bool = False, maximumWidth: int = 1920,
    ...
    boundingBoxLeft = float(identification[4]) / maximumWidth
    boundingBoxTop = float(identification[5]) / maximumHeight
    ...
    inputData.append(boundingBoxLeft)
    inputData.append(boundingBoxTop)
    ...
    positiveOutputData.append(float(positiveIdentification[4]) / maximumWidth)
    positiveOutputData.append(float(positiveIdentification[5]) / maximumHeight)
    ...
    try:
        ...
        negativeOutputData.append(float(negativeIdentification[4]) / maximumWidth)
        negativeOutputData.append(float(negativeIdentification[5]) / maximumHeight)
...
class StaticLSTM(nn.Module):
    def __init__(self, inputSize: int, hiddenSize: int, numberOfLSTMFCLayers: int, numberOfClassifierFCLayers: int):
        ...
        if numberOfLSTMFCLayers > 0:
            ...
            self.fc1.append(nn.Linear(in_features=self.hiddenSize, out_features=104))

        if numberOfClassifierFCLayers > 1:
            self.fc2 = []
            fcLayerSizes = np.linspace(2 * 104, 2, num=numberOfClassifierFCLayers + 1)
            ...
            self.fc2 = nn.Sequential(nn.ReLU(inplace=True),
                                     nn.Linear(in_features=104 * 2, out_features=2))
...
def train():
    ...
    for epoch in range(wandb.config.epochs):
        ...
        for trainingFilePath in glob.glob(generalPath + r"\DATA\LSTMTRACKLET\staticCamera\train\*.txt"):
...
def validation(epoch: int):
    ...
    with torch.no_grad():
        for validationFilePath in glob.glob(generalPath + r"\DATA\LSTMTRACKLET\staticCamera\val\*.txt"):
...
def test():
    ...
    with torch.no_grad():
        for testingFilePath in glob.glob(generalPath + r"\DATA\LSTMTRACKLET\staticCamera\test\*.txt"):
